...
void sl_button_action_(Button* b) {
    printdebug("Touching %d.", b->n._iu.nodraw_uint0);
}
...
void view_testfont_enter_(View* v) {
    NodeString* ns = node_asNodeStringOpt(v->n._iu.nodraw_node0);
...
View* View_createTestFont(void) {
    ...
    GlyphMap_setDefault((GlyphMapInit) {
        .nearest = true,
        .customChars_count = 1,
    ...
    // Lettre
//    DrawableChar* dc = DrawableChar_create(&v->n, (Character) { .c_str = "j" }, 0, 0.7, 0.5, 0);
//    Drawable_createTestFrame(&v->n,  0, 0.7, 2*node_deltaX(&dc->n), 2*node_deltaY(&dc->n));
    // String
    NodeStringInit str = {
        .c_str = "Bonjour pf" ,
        .maxCountOpt = 60,
    ...
    ns = NodeString_create(&v->n, str, 0, 0, 2, 0.4, 0, 0);
    ns->n.renderer_updateInstanceUniforms = nodestring_renderer_updateIUsMoving;
    ns->n._iu.nodraw_float0 = 0.5f;
    ns->n._iu.nodraw_float1 = 5.0f;
    v->n._iu.nodraw_node0 = &ns->n;
    Drawable_createTestFrame(&v->n,  0, 0, 2*node_deltaX(&ns->n), 2*node_deltaY(&ns->n));

    // String drawable
    coq_Font const *cf = GlyphMap_getDefaultFont();
    Drawable *d = Drawable_createString(&v->n, str.c_str, cf, 0, -0.6, 0.4, 0);
    Drawable_createTestFrame(&v->n, 0, -0.6, 2*node_deltaX(&d->n), 2*node_deltaY(&d->n));
    Drawable_createTestFrame(&v->n, 0, -0.6, d->n.sx, d->n.sy);
...
Root* Root_initAndGetProjectRoot(void) {
    ...
    fl_array_set(my_root.back_RGBA, color4_orange.f_arr, 4);
