...
void      drawable_last_setTile(uint32_t i, uint32_t j) {
    if(_Drawable_last == NULL) {
        printwarning("No last drawable.");
        return;
    }
    uint32_t m = texture_m(_Drawable_last->tex);
    uint32_t n = texture_n(_Drawable_last->tex);
    _Drawable_last->n.piu.i = (float)(i % m);
    _Drawable_last->n.piu.j = (float)((j + i / m) % n);
}
void      drawable_last_setEmph(float emph) {
    ...
    _Drawable_last->n.piu.emph = emph;
}
