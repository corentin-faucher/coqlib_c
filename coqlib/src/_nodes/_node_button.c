...
Button* Button_create(Node* refOpt, Root* root, void (*action)(Button*),
    ...
    Button* b = _Node_createEmptyOfType(node_type_nf_button, sizeof(Button),
                                       flags, refOpt, 0);
...
void _buttonhov_startHovering(Button* b) {
    ...
    timer_scheduledNode(&h->timer, 350, false, &h->n, _buttonhov_showPopMessage);
}
...
void       _buttonhoverable_init(ButtonHov* h, Root* root, void (*action)(Button*),
    ...
    // S'assurer de d'annuler le timer.
...
ButtonHov* ButtonHoverable_create(Node* refOpt, Root* root, void (*action)(Button*),
    ...
    ButtonHov* h = _Node_createEmptyOfType(node_type_nf_button, sizeof(ButtonHov),
                                       flags, refOpt, 0);
...
void _buttonsecure_grab(Button* sel, Vector2 pos) {
    ...
    timer_scheduledNode(&bt->timer, (int64_t)(bt->spi.holdTimeSec * 1000.f), false,
...
ButtonSecure* ButtonSecure_create(Node* refOpt, Root* root, void (*action)(Button*),
    ...
    ButtonSecure *sec = _Node_createEmptyOfType(node_type_nf_button, sizeof(ButtonSecure),
                                       flags, refOpt, 0);
    // Init as button
