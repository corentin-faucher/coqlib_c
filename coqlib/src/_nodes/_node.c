...
{
    return _Node_createEmptyOfType(node_type_node, sizeof(Node), 0, NULL, 0);
}
// Constructeur usuel.
Node* Node_create(Node* const refOpt, const float x, const float y, const float w, const float h,
    ...
    Node *node = _Node_createEmptyOfType(node_type_node, sizeof(Node), flags, refOpt, node_place);
    node->x = x;
    node->y = y;
    node->w = w;
    node->h = h;
...
extern void timer_scheduledNode(Timer** timerRefOpt, int64_t deltaTimeMS, bool isRepeating,
