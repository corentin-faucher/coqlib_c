...
bool sq_goRight(Squirrel *sq) {
    if(sq->pos->_littleBro == NULL) return false;
...
bool sq_goLeft(Squirrel *sq) {
    if(sq->pos->_bigBro == NULL) return false;
...
bool sq_goDown(Squirrel *sq) {
    if(sq->pos->_firstChild == NULL) return false;
...
bool sq_goDownLast(Squirrel *sq) {
    if(sq->pos->_lastChild == NULL) return false;
...
bool sq_goUp(Squirrel *sq) {
    if(sq->pos->_parent == NULL) return false;
...
bool sq_goToNextToDisplay(Squirrel *sq) {
    ...
    if(sq->pos->_firstChild && (sq->pos->flags & (flag_show|flag_parentOfToDisplay))) {
