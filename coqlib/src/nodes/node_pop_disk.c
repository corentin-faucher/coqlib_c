...
void _popdisk_close(Node* nd) {
    ...
    timer_scheduled(&pop->timer, 500, false, nd, node_tree_throwToGarbage);
}
...
void PopDisk_spawn(Node* const refOpt, PopDisk** const refererOpt,
    ...
    // Node init
    PopDisk* pop = Node_createEmptyOfType_(node_type_flag_drawable, sizeof(PopDisk),
                                       0, refOpt, 0);
    pop->n.x = x;      pop->n.y = y;
...
void  _popmessage_close(Node* n) {
    ...
    timer_scheduled(&pm->timer, 500, false, n, node_tree_throwToGarbage);
}
...
void   PopMessage_spawn(Node* refOpt,
    ...
    // Node init
    PopMessage *pm = Node_createEmptyOfType_(node_type_n_fluid, sizeof(PopMessage),
                                       0, refOpt, 0);
    Node* n = &pm->n;
    ...
    // PopMessage init
    timer_scheduled(&pm->timer, (int64_t)(timeSec*1000.f), false, n, node_tree_close);
