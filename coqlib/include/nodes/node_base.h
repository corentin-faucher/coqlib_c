...
// Même chose que les `flags_fluidRelative` dans `node__flags.h` gardés en mémoire pour les Fluid.
enum {
...
/// Permet d'aligner le noeud (à droite/gauche). Voir les `relatives_...` ci-haut ou `flags_fluidRelative`...
/// Dans le cas des Fluid, `node_setXYrelatively` est callé par open et reshape
/// et l'aligement se fait par rapport à la positien par défaut.
...
__attribute__((deprecated("utiliser `coq_calloc` + `node_init_`.")))
void* Node_createOfType_Obsolete_(const uint32_t type, const size_t size, const flag_t flags,
                              Node* const refOpt, const uint8_t node_place) ;
/// Le dernier noeud init qui n'est pas une feuille (i.e. pas drawable)
extern Node* node_last_nonLeaf;
