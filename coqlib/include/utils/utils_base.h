...
#define COQ__FILENAME__ (strrchr(__FILE__, '/') ? strrchr(__FILE__, '/') + 1 : __FILE__)
#ifdef DEBUG
...
printf("❌ Error: "format" -> %s line %d", ##__VA_ARGS__, COQ__FILENAME__, __LINE__), print_trace_(4)
#define printwarning(format, ...) \
printf("⚠️ Warn.: "format" -> %s line %d", ##__VA_ARGS__, COQ__FILENAME__, __LINE__), print_trace_(4)
#define printhere()\
print_here_(COQ__FILENAME__, __LINE__);
...
void    *coq_realloc_(void *__ptr, size_t __size, const char* filename, uint32_t line);

#ifdef COQ_ALLOC
#define coq_malloc(size) \
coq_malloc_(size, COQ__FILENAME__, __LINE__)
#define coq_calloc(count, size) \
coq_calloc_(count, size, COQ__FILENAME__, __LINE__)
#define coq_free(ptr) \
coq_free_(ptr, COQ__FILENAME__, __LINE__)
